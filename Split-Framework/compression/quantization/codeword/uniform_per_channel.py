...
class UniformPerChannelCodebookUInt8Codec:
    """Uniform 8-bit quantizer with one codebook per channel.

    This mirrors the existing uniform codebook quantizer, but computes a
    separate 256-level uniform codebook for each channel instead of using a
    single codebook for the entire tensor.

    By default, channel axis 1 is used for tensors with rank >= 2, which fits
    the common NCHW activation layout in this codebase. For rank < 2 tensors,
    axis 0 is used.
    """
    ...
    def _codec_name(self) -> str:
        return f"uniform_per_channel_codebook_uint{int(self.num_bits)}"
    ...
    def encode(self, x: torch.Tensor) -> Payload:
        ...
        base = torch.linspace(0.0, 1.0, steps=levels, dtype=torch.float32).unsqueeze(0)
        codebook = x_min.unsqueeze(1) + (x_max - x_min).unsqueeze(1) * base

        q_flat = torch.zeros_like(x_flat, dtype=torch.uint8)
        non_degenerate = x_max != x_min
        if bool(non_degenerate.any()):
            step = (x_max[non_degenerate] - x_min[non_degenerate]) / float(levels - 1)
            idx = torch.round((x_flat[non_degenerate] - x_min[non_degenerate].unsqueeze(1)) / step.unsqueeze(1))
            idx = torch.clamp(idx, 0, levels - 1).to(dtype=torch.uint8)
            q_flat[non_degenerate] = idx
