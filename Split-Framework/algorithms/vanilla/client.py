"""Client-side model/trainer for the `vanilla` algorithm variant."""
...
class SplitNNClient():
    ...
    def forward_pass(self):
        ...
        if self.args["save_acts_step"] >0 and self.step<=1 and self.phase=="validation" and self.epoch_count % self.args["save_acts_step"] == 0:
            ...
            a=a.cpu().detach()
            a=a.numpy()
            f=open("./saved_weights/acts/Vanilla/A_{}_E_{}_C_{}.txt".format(self.args["partition_alpha"],self.epoch_count,self.rank),"w")
    ...
    def print_com_size(self, com_manager):
        # Keep the original counters, but add a breakdown by message category/type.
