...
from sla_predictor import _default_dataset_root, build_dataset
...
class SplitClientNet(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, dropout: float, cut_layer: int) -> None:
        super().__init__()
...
class SplitLearningClient:
    def __init__(self, args: argparse.Namespace, logger: logging.Logger) -> None:
        ...
        self.train_loader, self.test_loader, input_dim = self._prepare_data(args)
        self.model = SplitClientNet(
            input_dim=input_dim,
    ...
    def _prepare_data(self, args: argparse.Namespace) -> tuple[DataLoader, DataLoader, int]:
        dataset_root = Path(args.dataset_root).expanduser().resolve()
        msrtmcr_dir = (dataset_root / args.msrtmcr_subdir).resolve()

        ds = build_dataset(
            msrtmcr_dir=msrtmcr_dir,
            label_rt_col=args.label_rt_col,
            sla_rt_threshold_ms=args.sla_rt_threshold_ms,
        ...
        n = int(ds.x.shape[0])
        if n < 10:
            raise RuntimeError(f"Dataset too small after sampling (n={n}).")

        perm = torch.randperm(n, generator=torch.Generator().manual_seed(args.seed))
        split = int(n * float(args.train_split))
        ...
        return train_loader, test_loader, int(x_train.shape[1])
    ...
    async def _evaluate(self, ws: WebSocketClientProtocol) -> Dict[str, Any]:
        ...
        for features, labels in self.test_loader:
            ...
            idx = (labels_cpu.to(torch.int64) * 2 + preds.to(torch.int64)).view(-1)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Split learning client for Alibaba SLA violation prediction")
    ...
    p.add_argument("--msrtmcr-subdir", type=str, default="data/MSRTMCR")
    p.add_argument("--label-rt-col", type=str, default="providerrpc_rt")
    p.add_argument("--sla-rt-threshold-ms", type=float, default=50.0)
    p.add_argument("--max-rows", type=int, default=200_000)
    p.add_argument("--max-tarballs", type=int, default=0)
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    logger.info(
        "client device=%s cut_layer=%d epochs=%d batch=%d",
        client.device,
        args.cut_layer,
        args.num_epochs,
        args.batch_size,
